...
DWORD CMyInternetSession::GetWebFile(LPCTSTR pstrAgent, LPCTSTR lpstrServer, int nPort,CString strPathName)
{
	...
	try {		
		...
		if(dwRet == HTTP_STATUS_OK)
			...
			filepath.Format("%s\\%s",m_strDownloadDirectory,strFName);
...
int CMyInternetSession::SetDownloadDirectory(CString strDirectory)
{
	m_strDownloadDirectory = strDirectory;

	//create directory
	return CreateDirectory(m_strDownloadDirectory, NULL);
}

CString CMyInternetSession::GetDownloadDirectory() {
